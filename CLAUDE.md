...
49. **Day-color WAIT + same-theme stacking WAIT + proxy-vol honesty — an entry recommendation may never say "do it today" when TODAY is the wrong day, the risk is already on in a correlated name, or the vol payment was never truly measured.** George (2026-08-14), after the briefing rendered "A- (80) WDC — SELL 1× $450P … RSI 48 late-band · RVr 84 ✓ — 🏁 Entry: A- — strong setup… Enter per plan" on a GREEN day (+1.5%) while he held the SNDK $1230P (WDC = SanDisk's former parent, same Memory & Storage theme): "make that change so that the briefing is not telling me to do it today. It's misleading." Three failures in one card: rule #44's day-color discipline lived in the PRIME conjunction and the context component but never gated the WEIGHTED tier's "Enter per plan"; the rule-#40 overlap check is per-NAME only, so a held put on the same THEME (SNDK vs WDC — one Memory & Storage flush assigns both) sailed through; and "Enter per plan" read unqualified while the vol component was the RVr realized-vol PROXY, not true chain IVr. All three fixes run through the canonical machinery (rule #48 — never a parallel path): (a) **STEP 2½ DAY-COLOR hard-WAIT** in `evaluate_entry` — a new CSP on a measured GREEN day / new CC on a measured RED day → verdict WAIT, never ENTER, with the measured move ("🟥 wait for a red day — WDC +1.5% today; sell puts into weakness (card rule)" / the 🟩 CC mirror). Day color = the vintage guard's live/broker drift reference vs the technicals close (the same rule-#47 resolution); NO reference → NO fabricated color (fail-open, rule #19). Config `entry_algorithm.day_color: {enabled, green_min_pct: 0.3, red_min_pct: -0.3}` — moves inside ±0.3% are neutral (neither side waits). (b) **SAME-THEME STACKING** (`analysis/theme_stacking.py`, single source of truth): ticker → theme mapping from the thematic scout's `theme_universes.yaml` anchors (a ticker may sit in multiple themes); a NEW CSP sharing ANY theme with an underlying where a short put is HELD → WAIT (config `entry_algorithm.theme_stacking: {enabled, mode: wait}`) with the held contract + shared theme named ("⛔ same-theme put already held — SNDK $1230P (Memory & Storage); stacking correlated assignment risk"). Wired in `evaluate_entry` STEP 2 right after the per-name rule-#40 overlap check; names in no mapped theme → no check (fail-open); same-ticker held puts stay rule #40's; CC writes exempt (share-backed). Held puts on diversified mega-cap / index names (`entry_algorithm.theme_stacking.exempt_held_tickers`, defaulting to the Tier A core union + SPY/VOO/QQQ) never trigger the same-theme WAIT — an AMZN put is not a robotics bet (the 2026-08-17 CGNX exclusion) — while pure-play held names (SNDK, MU, WDC, …) still do, and the exemption applies only to the HELD side, never the candidate. (c) **PROXY-VOL HONESTY** in the entry message: when the grade's vol component used the RVr proxy (not true chain IVr), the ENTER message renders "Enter per plan — verify true IVr at the broker (vol measured by RVr proxy)" — message-level only, grade/letter unchanged (💎 PRIME already excludes proxies). Because both gates live inside `evaluate_entry`, candidate cards, Best Setups, and the "🧮 Entry Algorithm Conformance" panel all inherit: the 🏆 spotlight demotes day-color/theme WAITs out of the ENTER list into the visible ⏸ exclusions ("⏸ WDC A- (80) — excluded: 🟥 wait for a red day…", rule #24), and the candidate surface renders a "⏸ Held back by the entry algorithm — wait" bucket. Management/rolls/closes untouched; `enabled: false` per flag → byte-identical legacy. Sources: `analysis/entry_algorithm.py` (STEP 2½ + theme wiring), `analysis/theme_stacking.py` (mapping + check), `analysis/setup_grade.py::_message` (caveat) + `collect_best_setups` (spotlight demotion), `steps/candidate_research.py` (wait bucket), `config/briefing.yaml` → `entry_algorithm`. Tests: `tests/test_rule49_wdc_day_color_theme.py` (docstrings quote "It's misleading").

50. **Tier A covered calls are ALGORITHM-GATED, not banned — when `covered_call_tiers.tier_a.algorithm_gated.enabled` is true, EVERY Tier A holding is CC-eligible and the CANONICAL entry algorithm (rule #48) is the green light; the per-name whitelist is obsolete; two compounder protections survive: ≤50% coverage and the tax-aware LTCG block.** George (2026-08-14): "I want to relax our covered calls strategy... we have long-term stocks notion... we should allow writing covered calls for ALL of them and incorporate that in the code, in the algorithm. So when there is a good [wheel] formula for selling a covered call based on RSI, IV rank, resistance, and strike, and everything included in the recommendation obviously, based on my holdings." The design, precisely: (a) **eligibility** — `position_tiers.is_cc_enabled_for_tier` returns True for ALL Tier A tickers when `algorithm_gated` is enabled (`willing_to_write_cc_on` becomes obsolete but is still honored when the block is absent/disabled — backward compatible; the rule-#35 engineered mode stays unchanged as an alternative path); rules #29/#34's Tier A bans are SUPERSEDED while the flag is on. (b) **the green light comes from `evaluate_entry(side='cc')`** — strategy_upgrades Type D consults the rule-#48 evaluator for every Tier A holding with ≥100 shares in one account; verdict ENTER is REQUIRED for the actionable ticket (which already enforces the RSI wait-for-strength band, IVr with proxy honesty, resistance-anchored strike via S/R discipline, GREEN-day color per rule #49, the B floor, and vintage-resolved RSI per #47 — no parallel logic, ever). WAIT/BLOCKED verdicts render in the wait-for-strength section with the evaluator's measured reason ("⏸ GOOG — CC waits: RSI 51 below 60" style); a missing live chain FAILS CLOSED (no actionable Tier A write on a rule-of-thumb estimate — rule #10). (c) **the algorithm-gated envelope** (config `covered_call_tiers.tier_a.algorithm_gated`, all-config defaults): min_otm_pct 10 · max_delta 0.15 · coverage_cap_pct ≤ 50 of round lots (at least half the upside stays uncapped) · DTE 21-45 monthly · earnings clear · `tax_aware_assignment_block: true` — never a coverage that risks a lot inside the 60-day pre-LTCG window (`position_tiers.tax_aware_cc_lot_adjustment`, the rule-#29 machinery: at-risk lots cap `contracts_writable` to the LTCG-safe round lots; zero safe lots → wait-list demotion with the measured note; no lot data → no adjustment, rule #19). Tier B/C envelopes unchanged. (d) **surfaces** — the existing Type D composer + panel (extended, not forked): ENTER renders "✅ READY TO WRITE" with the 🧮 one-liner (grade + 💎 when prime), real chain quote, measured delta, contracts capped by coverage + per-account writability; WAIT/BLOCKED renders "⏸/⛔ ENTRY ALGORITHM" with the reason; flows into Best Setups + digest via the existing grade pools. Sources: `analysis/position_tiers.py` (`algorithm_gated_settings`, `cc_settings_for_tier` overlay, `is_cc_enabled_for_tier`, `tax_aware_cc_lot_adjustment`), `steps/strategy_upgrades.py` (Type D consult), `render/strategy_upgrades_panel.py` (badges + reason lines), `config/briefing.yaml` → `covered_call_tiers.tier_a.algorithm_gated`. Tests: `tests/test_tier_a_algorithm_gated_cc.py` (docstrings quote George's directive).

51. **Momentum-hold overlay + earnings basis-cushion — a yield-motivated short-put close never fires while the underlying is measurably ripping; the stall trigger IS the exit; risk closes always override; and a willing-owner fortress (basis cushion ≥ 25%) holds THROUGH the print instead of closing before it.** George (2026-08-17): "I feel like we're trying to get out of our options a little too early. It feels like IREN is ripping. SNDK was ripping... If something is ripping and it didn't get out to RSI, say, to 70 or something. Do I really need to close? Validate it, and let's make sure we squeeze as much as possible out of these options." Validated evidence: Friday's briefing said close SNDK at +33%; held through the rip, Monday +62% (+$1,280 in 3 sessions). The design, precisely: (a) **the ride** — `analysis/momentum_hold.py::momentum_ride(review, snapshot_data, config)` defers a yield-motivated close/TP on a SHORT PUT to a visible `🏇 RIDE — momentum hold` action item (rule #24 — HOLD-class, never hidden) ONLY when ALL of: underlying trend UP (live/day move ≥ 0 AND 3-session move > 0, measured from the rule-#46/#47 live/broker drift reference against the technicals close series — unmeasurable momentum fails OPEN to NO ride, never fabricated, rule #19); vintage-resolved RSI < `rsi_stall` (default 65, `resolve_new_open_rsi` — no ride on stale/unverified RSI); OTM cushion ≥ `min_otm_pct` (default 10%, the gamma-safety floor); capture < `hard_ceiling_pct` (85% — at/above always closes). (b) **risk always overrides momentum** — `momentum_hold.risk_exempt` reuses the existing exemption lists: the over-cap obligation-inclusive concentration exemption (the 2026-08-17 SNDK/SOXX fix, `redeploy_path.over_cap_risk_exemption`) ALWAYS wins, plus loss-stop/crash/tail-risk/earnings-imminent guardrail cells, CLOSE NOW / CLOSE_URGENT verdicts, earnings inside the contract window (`exit_cost.gtc_min_days_to_earnings`, 30d), and the gamma escape (DTE ≤ 10 @ ≥ 30%). (c) **the stall trigger IS the exit** — the ride line renders the measured trigger ("close on FIRST RED DAY or RSI ≥ 65 or 85% capture"); the next briefing where the underlying prints red or RSI reaches the stall, the returning CLOSE carries "(momentum stalled — take it)" appended. Riders are never banked: the Money Plan counts them under Blocked money ("N winner(s) riding momentum"), `net_option_cash._DEMOTION_FLAGS` includes `_momentum_ride` (the fold-in guard), and Risk Alerts rewrite to "🏇 RIDING MOMENTUM" (the AVGO two-surfaces lesson). (d) **earnings basis-cushion (the IREN case)** — in the CLOSE BEFORE EARNINGS path (`render/panels.py::_one_voice_take_profit_lines`), when the assignment-basis cushion (spot vs basis = strike − entry premium) is ≥ `earnings_hold_min_cushion_pct` (default 25%), the single voice becomes "**HOLD THROUGH EARNINGS — willing owner**: basis $28.36 is 39% below spot; a -25% print still assigns above basis" (mathematically backed: spot × (1 − thr) ≥ basis ⟺ cushion ≥ thr) with the extrinsic/remaining premium that stays yours if held, "place NO GTC through the print", and the protect-the-capture close as explicitly SUBORDINATE counter-case context; cushion below threshold / unmeasurable → close-before-earnings exactly as today. Real-data validation (2026-08-17 book): IREN $47P (+45%, cushion 39%, earnings 10d) flips CLOSE BEFORE EARNINGS → HOLD THROUGH EARNINGS; SOXX $520P still closes (over-cap risk overrides momentum; 8% cushion under the floor anyway); SMH $710C unchanged (overlay is put-only — calls keep legacy behavior). Measured numbers only on every line (rule #19); `enabled: false` → byte-identical legacy. Sources: `analysis/momentum_hold.py` (single source of truth), `render/panels.py` (CLOSE WINNERS block #2 + matrix CLOSE_FOR_PROFIT block #4 + the willing-owner earnings branch), `render/money_plan.py` (riding count), `analysis/net_option_cash.py` (fold-in guard), `config/briefing.yaml` → `momentum_hold`. Tests: `tests/test_momentum_hold.py` (21 tests, docstrings quote George's directive; the SNDK-Friday fixture rides, the over-cap Monday fixture closes).

    **Willing-owner roll downgrade (2026-08-18 NOK extension of the same doctrine).** George, on the NOK 10-lot card "🚨 URGENT — EXECUTE ROLL NOK_PUT_11_20261218 — Calendar roll... +$115 net credit" with 122 DTE and assignment basis $8.44 vs spot $10.29: "Why should I roll Nokia if it's in December? It's kind of hard." The strike-tested URGENT roll tier protects UNWILLING owners; for a WILLING owner a same-strike/down CREDIT roll is an optional income optimization, not urgent. When ALL of: short PUT · basis cushion (spot vs basis = strike − entry premium) ≥ `min_cushion_pct` · DTE ≥ `min_dte` · no earnings within `earnings_clear_days` · same-strike/down credit roll (never a loss-stop / crash / tail-risk / earnings-window / DEFENSIVE cell — those stay urgent, always), the action-list roll headline downgrades to "🔧 OPTIONAL — credit extension (no action required)" with the honest measured framing ("willing owner: basis $8.44 is 18% below spot $10.29; assignment in Dec is a win. Roll adds +$115/+28d if convenient; otherwise park the GTC at 50% ($1.28) or let it run." — rule #19, measured numbers only) and the FULL combo ticket retained on the card (rule #24). The credit-window state line stays (the honest "this offer expires" context); a debit-only window on a willing-owner position keeps the existing GTC/hold voice (exit-cost verdict / debit-cap machinery — a debit roll never downgrades to "optional income"). OPTIONAL items sort with the hold-class group (after executable, before deferred) and count under "+N hold". Config: `roll.willing_owner_downgrade {enabled, min_cushion_pct: 15, min_dte: 60, earnings_clear_days: 21}` — 15 (not 20) because the motivating NOK card measures (10.29 − 8.44)/10.29 = 18.0%; code default OFF = legacy byte-identical. Sources: `analysis/willing_owner.py` (single source of truth), `render/panels.py` (block #3 EXECUTE ROLL + block #4 roll-directive paths, `_HOLD_CLASS_HEAD_RE`). Tests: `tests/test_willing_owner_downgrade.py` (docstrings quote George's NOK question).

    **Directive rides (2026-08-26 MELI split-brain extension).** A position directive filed in the CANONICAL pipeline directives file — `skills/daily-portfolio-briefing/state/directives/index.yaml`, read by `steps/load_directives.py` Step 1.5 (a directive written only to `state/fable_advisor_memory.md` reaches Fable but NOT the deterministic pipeline: the 2026-08-26 briefing rendered "2. **CLOSE** MELI_PUT_1460_20270617 — +34% ($+3,435) … ⏳ IGNORED 3 DAYS" and banked it in the Money Plan while Fable said "tracking the exits, not re-litigating the close") — that carries a machine-readable `exit_conditions` block (`gtc_capture` / `gtc_limit_price` / `gtc_stretch_capture` / `red_day_pct` / `rsi_stall` / `max_capture` / `earnings_min_capture`; format documented in the index.yaml header) suppresses the recommendation-class CLOSE and instead TRACKS the directive's OWN exits with measured values every cycle: "🏇 RIDING (directive) — exits armed: GTC@75% $24.60 · stall on red day ≤-2% or RSI ≥ 70 · [today: MELI -0.9% · RSI 60 — no exit trigger fired]". When a stall condition fires the render states it LOUDLY and the exit line IS the action item ("[today: MELI -2.3% (red-day stall ≤ -2%) — STALL TRIGGER FIRED, exit per directive]" + the buy-to-close ticket); never banked in the Money Plan while riding (`_directive_ride` in `net_option_cash._DEMOTION_FLAGS`); unmeasured conditions render as unmeasured, never fabricated (rule #19). Sources: `analysis/directive_rides.py` (single source of truth), `steps/review_options.py` (directive short-circuit carries economics + payload), `render/panels.py` (action-list block #0). Tests: `tests/test_directive_rides.py`.
...
29. **Position tier framework — every holding is classified A/B/C and that tier gates covered-call writes + concentration caps. Tier A core compounders get NO CC recommendations EVER; concentration cap raised to ~22% (capping a conviction compounder at 10% defeats the long-term thesis).** (Tier A CC ban superseded by #50 when `algorithm_gated` is enabled.) Three tiers, three disciplines:
